...
Для вызова из любого локального проекта положите агентский токен в
`~/.devboard/client.env`:
...
Материализация в пользовательский кэш со скриншотами, но без повторного
скачивания аудио при готовой транскрипции:

```bash
python cli/devtask.py get DEV-52 --url https://current.trycloudflare.com --materialize --skip-audio
```

Кнопка «Промпт агенту» подставляет текущий URL DevBoard и вызывает
пользовательский `$devboard-task`, который выполняет этот handoff из рабочего
репозитория целевого проекта.

В карточке задачи есть кнопка «Скопировать промпт агенту»:

> $devboard-task
>
> Возьми DEV-52 из DevBoard: https://current.trycloudflare.com.
>
> Проект задачи: NeuroROP. Работай в текущем репозитории. Сначала изучи задачу,
> доступные вложения и текущий проект, затем обсуди со мной понимание, вопросы и
> план. Ничего не изменяй до моего явного разрешения.
